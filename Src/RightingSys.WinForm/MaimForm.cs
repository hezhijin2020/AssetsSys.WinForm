...
namespace RightingSys.WinForm
{
    public partial class MainForm :DevExpress.XtraBars.Ribbon.RibbonForm,AppPublic.Interface.IMainForm
    {
        ...
        private static List<RightingSys.Models.ACL_Role_Function> ListUserFuntions =null;
        ...
        public void SetFeatureButton(params FeatureButton[] btns)
        {
            if (btns != null)
            {
                ...
                for (int i = 0; i < btns.Length; i++)
                {
                    ...
                    this.SetButtonEnableByCode(code, true);//是否启用
                }
            }
        }
        /// <summary>
      ...
      /// <returns></returns>
        ...
        /// <param name="pDepartmentId">用户部门</param>
        /// <returns></returns>
        public List<RightingSys.Models.ACL_Role_Function> InitUserFunctionNew(Guid pSystemId, Guid pUserId, Guid pDepartmentId)
        {
            ...
            try
            {
                for (int i = 0; i < this.MainRibbon.Items.Count; i++)
                {
                    if ((this.MainRibbon.Items[i].Tag != null) && (this.MainRibbon.Items[i].Tag.ToString() != "NO") && (this.MainRibbon.Items[i] is BarButtonItem || this.MainRibbon.Items[i] is BarSubItem))
                    {
                        ...
                        {
                            this.MainRibbon.Items[i].Visibility = BarItemVisibility.Never;
                        }
                        else
                        {
        ...
        {
            MdiShow(new RightingSys.WinForm.SubForm.pgAssetsManagerForm.AssetsForm(), btnAssetsManager.Tag);
        }

        private void btnAssetsApply_ItemClick(object sender, ItemClickEventArgs e)
        {
            MdiShow(new RightingSys.WinForm.SubForm.pgAssetsManagerForm.AssetsApplyForm(), btnAssetsApply.Tag);
        }

        private void btnAssetsRefund_ItemClick(object sender, ItemClickEventArgs e)
        {
            MdiShow(new RightingSys.WinForm.SubForm.pgAssetsManagerForm.AssetsRefundForm(), btnAssetsRefund.Tag);
        }

        private void btnAssetsBorrow_ItemClick(object sender, ItemClickEventArgs e)
        {
            MdiShow(new RightingSys.WinForm.SubForm.pgAssetsManagerForm.AssetsBorrowForm(), btnAssetsBorrow.Tag);
        }

        private void btnAssetsReturn_ItemClick(object sender, ItemClickEventArgs e)
        {
            MdiShow(new RightingSys.WinForm.SubForm.pgAssetsManagerForm.AssetsReturnForm(), btnAssetsReturn.Tag);
        }
